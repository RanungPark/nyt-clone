...
const Headline = styled.div<{ booleanHeadline: boolean }>`
  ...
  border: 1px solid
    ${(props) =>
      props.booleanHeadline ? props.theme.blueSub : props.theme.glay};
  color: ${(props) =>
    props.booleanHeadline ? props.theme.blueMain : props.theme.glay};
  border-radius: 30px;
...
const PubDate = styled.div<{ booleanPubDate: boolean }>`
  border: 1px solid
    ${(props) =>
      props.booleanPubDate ? props.theme.blueSub : props.theme.glay};
  color: ${(props) =>
    props.booleanPubDate ? props.theme.blueMain : props.theme.glay};
  border-radius: 30px;
  padding: 6px 12px 4px;
  display: flex;
  align-items: center;
...
const Glocations = styled.div<{ booleanCountry: boolean }>`
  border: 1px solid
    ${(props) =>
      props.booleanCountry ? props.theme.blueSub : props.theme.glay};
  color: ${(props) =>
    props.booleanCountry ? props.theme.blueMain : props.theme.glay};
  border-radius: 30px;
  padding: 6px 12px 4px;
  display: flex;
  align-items: center;
    ...
    <Wrapper>
      <HeaderItems>
        <Headline onClick={handleClick} booleanHeadline={!!headline}>
          {headline ? (
            <img src={searchFill} alt="searchFill" />
          ) : (
        ...
        <PubDate onClick={handleClick} booleanPubDate={!!pubDate}>
          {pubDate ? (
            <img src={calendarCheckFill} alt="calendarCheckFill" />
          ) : (
        ...
        <Glocations
          onClick={handleClick}
          booleanCountry={!!filterCountry.length}
        >
          {filterCountry.length !== 0
            ? filterCountry[0] + " 외 " + (filterCountry.length - 1) + "개"
            : "전체 국가"}
        </Glocations>
      </HeaderItems>
    </Wrapper>
